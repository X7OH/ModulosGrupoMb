  ...
  implementation: function (discountRule, receipt, line, listener) {
    ...
    OB.Dal.find(OB.Model.DiscountFilterProduct, criteria, function (products) {
      receipt.get('lines').forEach(function (l) {
        if (products.indexOf(l.get('product')) !== -1) {
          // The product is in the list of candidates products
          if (!l.get('promotionCandidates')) {
            l.set('promotionCandidates', []);
          }
          if (l.get('promotionCandidates').indexOf(discountRule.id) === -1) {
            l.get('promotionCandidates').push(discountRule.id);
          }
        }
      });
      ...
      products.forEach(function (product) {
        ...
        if ((_.isUndefined(product.get('obdiscQty')) || _.isNull(product.get('obdiscQty'))) || product.get('obdiscIsGift')) {
          qtyToGetGift = 0;
        } else {
