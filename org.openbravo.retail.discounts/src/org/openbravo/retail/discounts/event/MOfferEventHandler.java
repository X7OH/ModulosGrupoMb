...
public class MOfferEventHandler extends EntityPersistenceEventObserver {
  ...
  public void onUpdate(@Observes EntityUpdateEvent event) {
    ...
    if (!checkPriceAdjustment(disc)) {
      throw new OBException(Utility.messageBD(new DalConnectionProvider(false),
          "OBDISC_DiscPriceAdjCheckAmtPerc", OBContext.getOBContext().getLanguage().getLanguage()));
    }
  }
  ...
  private boolean checkPriceAdjustment(PriceAdjustment disc) {
    if (("5D4BAF6BB86D4D2C9ED3D5A6FC051579".equals(disc.getDiscountType().getId()))
        && (disc.getDiscountAmount().compareTo(BigDecimal.ZERO) > 0 && disc.getDiscount()
