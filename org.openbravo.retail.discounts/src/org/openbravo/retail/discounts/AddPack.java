...
public class AddPack extends DalBaseProcess {
    ...
    if (lst_samePackLines.size() > 0) {
      ...
      for (Product packProduct : pack.getPricingAdjustmentProductList()) {
        ...
        if (lineTax == null) {
          ...
          msg.setType("Error");
          msg.setTitle("@Error@");
          msg.setMessage(OBMessageUtils.getI18NMessage("OBDISC_ErrorProductTaxConfiguration",
  ...
  private OrderLine calculateOrderLinePrices(OrderLine ol,
    ...
    if (priceList.isPriceIncludesTax()) {
      ...
      final BigDecimal netAmt = FinancialUtils.calculateNetAmtFromGross(ol.getTax().getId(), amt,
          precision, amt);
      ol.setUnitPrice(netAmt.divide(ol.getOrderedQuantity()).setScale(
          priceList.getCurrency().getPricePrecision().intValue()));
      ol.setLineNetAmount(netAmt.setScale(precision));
    } else {
