...
public class ArchTenthGuayaquilBankTXT extends DalBaseProcess {
  ...
  public void doExecute(ProcessBundle bundle) throws Exception {
    ...
    try {
      ...
      OBCriteria<SSPHTenthSettlement> obc = OBDal.getInstance()
          .createCriteria(SSPHTenthSettlement.class);
      obc.add(Restrictions.eq(SSPHTenthSettlement.PROPERTY_DOCUMENTNO, strDocumentNo.trim()));
      obc.setFilterOnReadableOrganization(false);
      obc.setMaxResults(1);
      SSPHTenthSettlement attach = (SSPHTenthSettlement) obc.uniqueResult();
      if (attach != null) {

        // Get the Payroll Ticket data
        // Obtener los datos de la Boleta de Nomina
        ArchTenthGuayaquilData data[] = ArchTenthGuayaquilData.select(conn, attach.getId(),
            ssfiBanktransferId);
        if (data != null && data.length > 0) {
          bundle.setResult(message);
          attach.setProcessCounter(attach.getProcessCounter().add(BigDecimal.ONE));
          // TODO: Save actual headers
          final DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
          final Date date = new Date();
          String number_process = attach.getProcessCounter().toString();
          ...
          try {
            for (ArchTenthGuayaquilData tenthData : data) {
              out.write(tenthData.cuenta);
              out.write(String.format("%-20s", ""));
              out.write(tenthData.cliente);
              out.write("\r\n");   // write new line
            }
