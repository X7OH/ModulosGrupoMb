...
OB.DS.HWResource = function (res, resorcedata) {
	  this.resource = res;
	  this.resourcedata = resorcedata;
	};
...
OB.DS.HWServer.prototype.print = function (template, params, callback, device) {
  ...
  if (template) {
    if (template.getData) {
      ...
      return me.print(data, params, callback, device);
      });
    } else {
    	return template;
...
OB.DS.HWServer.prototype._print = function (templatedata, params, callback, device) {
  ...
  try {
    ...
    if (computeddata.substr(0, 6) === 'jrxml:') {
      ...
      return this._send(computeddata, callback, device);
    }
  } catch (ex) {
...
OB.DS.HWServer.prototype._template = function (templatedata, params) {
  return params ? _.template(templatedata.resourcedata, params) : templatedata;
};
