...
public class Speya_OrderReady extends DalBaseProcess {
	private final Logger logger = Logger.getLogger(Speya_OrderReady.class);
	...
	protected void doExecute(ProcessBundle bundle) throws Exception {
		...
		String strJSON = Log.getJson();
		String UrlOrderPrepared = Log.getSpeyaUrlprepared();
		JSONObject JSON = new JSONObject(strJSON);
		String OrderToken = JSON.getString("token");

		try {
			String AuthToken = obtenerToken();
			String OrderRespons = actualizarEstadoOrden(Log, OrderToken, AuthToken, UrlOrderPrepared);

			if (OrderRespons.equals("OK")) {
				msg.setTitle(Utility.messageBD(conn, "Success", language));
				msg.setType("Success");
				msg.setMessage(OrderRespons + " - Preparacion Confirmada");
			} else {
...
//			OBDal.getInstance().flush();
	...
	public String actualizarEstadoOrden(SDELVRLogs Log, String TokenOrder, String token, String UrlOrderPrepared) {
		...
		try {
			...
			if (ResponseCode == HttpURLConnection.HTTP_OK) {
				...
				Log.setSpeyaListstates("prepared");
