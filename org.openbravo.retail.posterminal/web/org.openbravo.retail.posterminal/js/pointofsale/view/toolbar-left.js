  ...
  tap: function () {
    ...
    if (this.disabled === false) {
      ...
      if (!OB.MobileApp.model.get('isMultiOrderState') && this.model.get('order').getPaymentStatus().isNegative) {
        ...
        var orden = this.model.get('order');
        var documentNo = orden.get('documentNo');
        var terminal = orden.get('posTerminal$_identifier');

        if(userPOS.supervisor == null || userPOS.supervisor == 'null'){

            OB.UTIL.showConfirmation.display('', OB.I18N.getLabel('SRRATH_UnassignedSupervisor'), [{
          ...
          /*******************************************************************/
          /*           INICIO ENVIO DEL CORREO ACTUALIZACION PASSWORD        */
          /*******************************************************************/
          var process = new OB.DS.Process('ec.com.sidesoft.retail.reverse.authorization.sendMailReverse');
          process.exec({
            ...
            /*if(data.exception.status.tipo == 1){
              
              OB.UTIL.showConfirmation.display('', OB.I18N.getLabel('SRRATH_NoEmailSupervisor'), [{
                label: OB.I18N.getLabel('OBMOBC_LblOk')
              }]);
              return;             
              ...
              OB.UTIL.showConfirmation.display('', OB.I18N.getLabel('SRRATH_ConfigErrorEmail'), [{
                label: OB.I18N.getLabel('OBMOBC_LblOk')
              }]);
              return;

            }else if(data.exception.status.tipo == 3){
              
              OB.UTIL.showConfirmation.display('', OB.I18N.getLabel('SRRATH_ConfigError'), [{
                label: OB.I18N.getLabel('OBMOBC_LblOk')
              }]);
              return;

            }else if(data.exception.status.tipo == 4){

              OB.UTIL.showConfirmation.display('', OB.I18N.getLabel('SRRATH_NoEmailSend'), [{
                label: OB.I18N.getLabel('OBMOBC_LblOk')
              }]);
              return;
