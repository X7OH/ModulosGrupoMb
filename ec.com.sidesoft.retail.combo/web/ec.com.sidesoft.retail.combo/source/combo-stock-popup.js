    ...
    function executeCallToServer(changedModel) {
      ...
      serverCall.exec({
          ...
          if (data.exception) {
            ...
            if (data.qty <= 0 && changedModel.get('comboId')) {
              var lines = [changedModel];
              if (changedModel.get('comboId')) {
                lines = ComboModificationUtils.getLinesWithCombo(changedModel.get('comboId'), me.model.get('order'))
              }
              me.model.get('order').deleteLinesFromOrder(lines);
            } else {
