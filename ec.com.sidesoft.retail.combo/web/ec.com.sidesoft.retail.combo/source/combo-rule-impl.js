...
OB.Model.Discounts.discountRules[OB.OBCOMBO.comboRuleId] = {
  ...
  isCandidate: function (
    discountRule, receipt, line, isCandidateCallback, isNotCandidateCallback) {
    var familyCriteria = {
        // Query local DB to know detail about the rule
        ...
        OB.Dal.findUsingCache("ComboProductQuery", OB.Model.ComboProduct, productCriteria, function (products) {
          ...
          products.forEach(function (product) {
            ...
            for (i = 0; i < receipt.get("lines").length; i++) {
              ...
              if (
                currentLine.get("product").id === product.get("product")) {
  ...
  isCandidateFullReceipt: function (
    discountRule, combination, lineNo, isCandidateCallback, isNotCandidateCallback) {
    var familyCriteria = {
        // Query local DB to know detail about the rule
        priceAdjustment: discountRule.id
      },
  ...
  implementation: function (discountRule, receipt, line, listener, candidates) {
    ...
    function applyDiscount() {
      ...
      receipt.get("lines").forEach(function (l) {
        // if (l.get('comboId') !== discountRule.get('id')) {
        //   return;
        // }
        ...
        if (l.get("promotions") && l.get("promotions").length > 0) {
          for (i = 0; i < l.get("promotions").length; i++) {
            ...
            if (promotion.priority && priority !== null) {
              ...
              if (
                discountRule.get("discountType") === promotion.discountType && discountRule.id === promotion.ruleId) {
                receipt.removePromotion(l, discountRule);
              }
            }
          }
      ...
      while (!done && canApply) {
        for (i = 0; i < applyingFamilies.length; i++) {
          ...
          while (!completedFamily) {
            ...
            if (
              applyingFamilies[i].position === applyingFamilies[i].lines.length) {
      ...
      if (chunks !== 0 && canApply) {
        ...
        _.forEach(selectedLines, function (line) {
          ...
          if (OB.OBCOMBO.discountType.fixPrice === discountType) {
            discountAmt = OB.DEC.mul(
              OB.DEC.sub(price, family.get("fixedPrice")), line.units);
            ...
            discountAmt = OB.DEC.mul(
              OB.DEC.div(
                OB.DEC.toBigDecimal(price).multiply(
                  OB.DEC.toBigDecimal(family.get("percentage"))), 100), line.units);
          } else if (OB.OBCOMBO.discountType.fixDiscount === discountType) {
            discountAmt = OB.DEC.mul(family.get("fixedDiscount"), line.units);
          } else if (
            OB.OBCOMBO.comboFixPrice.discountType.comboFixPrice === discountType) {
            lineNum += 1;
            if (lineNum < selectedLines.length) {
              discountAmt = OB.DEC.mul(
                OB.DEC.div(
                  OB.DEC.mul(promotionAmt, OB.DEC.mul(price, chunks)), total), lineUnits);
          ...
          var finalLine = receipt.get('lines').models.filter(function (internalLine) {
            return internalLine.get('id') === receiptLine.get('id');
          })
          if (finalLine[0].get('comboId') === discountRule.get('id')) {
        ...
        completedArgs = {
          alerts: OB.I18N.getLabel("OBPOS_DiscountAlert", [
            line.get("product").get("_identifier"), discountRule.get("printName") || discountRule.get("name")
          ])
        };
      }
    ...
    //TODO delete all not compatible discounts
    if (line.get('comboId')) {
      promotionCandidates = [line.get('comboId')];
    } else {
      promotionCandidates = [];
    }
  ...
  addComboPromotion: function (receipt, line, promotion) {
    receipt.addPromotion(line, promotion.rule, promotion.definition);
  },
  /**
  ...
  addProductToOrder: function (order, productToAdd) {
    ...
    var lineWithCombo = false;
    if (order.get('lines').length > 0) {
      var lines = order.get('lines');
      for (var i = 0; i < lines.models.length; i++) {
        var line = lines.models[i];
        var combo = ComboModificationUtils.isAffectedByComo(line);
        if (combo && combo.ruleId === productToAdd.id) {
          lineWithCombo = true;
          break;
        }
      }
    ...
    if (lineWithCombo) {
      popupInstance.loadFamiliesWithValues(
        productToAdd.name, productToAdd.id, order);
    } else {
