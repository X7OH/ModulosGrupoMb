...
public class FileGenerationEcuador extends AbstractFileGeneration
  ...
  public String generateFile(Invoice invoice, String rootDirectory, String lang) throws Exception {
    ...
    OBCriteria<Organization> ObjOrganization1 = OBDal.getInstance()
        .createCriteria(Organization.class);
    OrganizationType objOrganizationType1 = OBDal.getInstance().get(OrganizationType.class, "1");
    ObjOrganization1
        .add(Restrictions.eq(Organization.PROPERTY_ORGANIZATIONTYPE, objOrganizationType1));

    boolean boolDiscountbypricelist = ObjOrganization1.list().get(0).isEeiDiscountbypricelist();
    ...
    for (InvoiceLine detalleObj : lines) {
      ...
      if (strDescription == null || strDescription.trim().equals("")) {
        throw new OBException(
            "El producto (" + detalleObj.getProduct().getName() + ") no tiene descripción.");
      }
      ...
      precioTotalSinImpuesto
          .appendChild(doc.createTextNode(formateador.format(PrecioTotalSinImpuesto).toString()));
    ...
    if ((((invoice.getDescription() != null && !invoice.getDescription().trim().equals(""))
        ...
        || (invoice.getBusinessPartner().getName2() != null) || (strWithholdingAgent != null)) {

      Element infoAdicional = doc.createElement("infoAdicional");
      rootElement.appendChild(infoAdicional);

      if (strWithholdingAgent != null) {
        Element campoAdicional0 = doc.createElement("campoAdicional");
        campoAdicional0.setAttribute("nombre", "agenteRetencion");
        campoAdicional0.appendChild(doc.createTextNode(strWithholdingAgent));
        infoAdicional.appendChild(campoAdicional0);
      }
