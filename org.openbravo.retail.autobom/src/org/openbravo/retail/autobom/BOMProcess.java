...
public class BOMProcess {
  ...
  private JSONArray productionList = new JSONArray();
  ArrayList<MaterialTransaction> materialTransactionList = new ArrayList<MaterialTransaction>();

  public void createProcessBOM(JSONObject jsonorder) throws OBException, JSONException {
    ...
    try {
      ...
      Date date = OBMOBCUtils.calculateServerDate(jsonorder.getString("orderDate"),
      ...
      for (int i = 0; i < lines.length(); i++) {
        ...
        BigDecimal qty = new BigDecimal(line.getDouble("qty"));
        if (product.isBillOfMaterials() && product.isObbomAutogeneratebom() && qty.signum() == 1) {
          ...
          createRecurrencyBOM(product, qty, organization, date);
        }
      }
  ...
  private void createRecurrencyBOM(Product product, BigDecimal qty, Organization organization,
    ...
    for (ProductBOM productBOM : product.getProductBOMList()) {
      ...
      if (bomProduct.isBillOfMaterials() && bomProduct.isObbomAutogeneratebom()) {
        ...
        BigDecimal stock = getStock(productBOM, organization) == null ? BigDecimal.ZERO
            : getStock(productBOM, organization);
        if (stock == null || stock.compareTo(BigDecimal.ZERO) == -1) {
          stock = BigDecimal.ZERO;
        }
        BigDecimal totalQty = qty.multiply(productBOM.getBOMQuantity()) == null ? BigDecimal.ZERO
            : qty.multiply(productBOM.getBOMQuantity());

        // **************************************************************
        // INICIO SE COMENTA LA VALIDACION PARA QUE SIEMPRE PRODUZCA
        // PRIMERA PASADA STOCK 0.compareTo(0.024)  == -1 (ENTRA AL IF SE PRODUCE)
        // SEGUNDA PASADA STOCK 0.024.compareTo(0.024)  == 0 (NO ENTRA AL IF NO SE PRODUCE)
        // **************************************************************                    
        // if (stock.compareTo(totalQty) == -1) {
          // SE CAMBIA ESTO totalQty.subtract(stock)
          ...
          // POR ESTO totalQty
          bomProducts.put(productBOM.getBOMProduct(), totalQty);
          createRecurrencyBOM(productBOM.getBOMProduct(), totalQty, organization,
              date);              
        // }
        // **************************************************************
        // FIN SE COMENTA LA VALIDACION PARA QUE SIEMPRE PRODUZCA
        // **************************************************************
      }
    }
    if (!bomProducts.isEmpty()) {
  ...
  private void addProductionList(String productionTransactionId) {
    this.productionList.put(productionTransactionId);
  }
...
//    }
//    
//    System.out.println(selectSD.toString());
//
...
//    //if (stock == null) {
//    //  throw new OBException(
//    //      "Error in AutoBOM when getting stock for product " + productBOM.getBOMProduct().getName());
//    //}
//
//    return stock;
//
