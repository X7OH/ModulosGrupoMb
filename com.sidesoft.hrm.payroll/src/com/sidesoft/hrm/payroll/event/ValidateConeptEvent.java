...
public class ValidateConeptEvent extends EntityPersistenceEventObserver {
  ...
  private static Entity[] entities = { ModelProvider.getInstance().getEntity(Concept.ENTITY_NAME) };
  ...
  private void ReturnQuery(Concept Strserach) {
    ...
    OBCriteria<Concept> ConceptsCritria = OBDal.getInstance().createCriteria(Concept.class);
    ...
    // throw new OBException(Utility.messageBD(conn, "ERROR=IMPUESTO A LA RENTA YA CREADO",
    // language));
