...
public class ValidateDateIessRateLine extends EntityPersistenceEventObserver {
  ...
  private static Entity[] entities = {
      ModelProvider.getInstance().getEntity(sspr_iessrateline.ENTITY_NAME) };
  ...
  private void ReturnQuery(sspr_iessrateline Strserach) {
    ...
    OBCriteria<sspr_iessrateline> UserCriteria = OBDal.getInstance()
        .createCriteria(sspr_iessrateline.class);
    UserCriteria.add(Restrictions.or(
        Restrictions.and(Restrictions.lt(sspr_iessrateline.PROPERTY_VALIDFROM, V_StartDate),
            Restrictions.ge(sspr_iessrateline.PROPERTY_VALIDTO, V_StartDate)),
        Restrictions.and(Restrictions.lt(sspr_iessrateline.PROPERTY_VALIDFROM, V_EndDate),
