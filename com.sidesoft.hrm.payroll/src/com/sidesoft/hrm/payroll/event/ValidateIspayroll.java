...
public class ValidateIspayroll extends EntityPersistenceEventObserver {
  ...
  private void ReturnQuery(BankAccount Strserach) {
    ...
    BusinessPartner partner = OBDal.getInstance().get(BusinessPartner.class,
        Strserach.getBusinessPartner().getId().toString());
    ...
    if (tm >= 1 & Strispayroll == true & Strisemployee == true) {
      ...
      throw new OBException(Utility.messageBD(conn,
          "@Aqui esta el mesaje de error insert@ " + String.valueOf(tm), language));
      // throw new OBException(Utility.messageBD(conn, "@sspr_ispayroll_unique@" ));
      // }else{

      // throw new OBException(Utility.messageBD(conn, "@Aqui esta el mesaje de error insert@ " +
      // String.valueOf(tm),language ));
    }
  ...
  private void ReturnQueryUpdate(BankAccount Strserach) {
    ...
    if (tm > 1 & Strispayroll == true & Strisemployee == true) {
      ...
      throw new OBException(Utility.messageBD(conn,
          "@Aqui esta el mesaje de error update@ " + String.valueOf(tm), language));
      // throw new OBException(Utility.messageBD(conn, "@sspr_ispayroll_unique@" ));
      // }else{
      // throw new OBException(Utility.messageBD(conn, "@Aqui esta el mesaje de error update@ " +
      // String.valueOf(tm),language ));
