...
public class GiftCardHookUtils {
  ...
  private static final String PAYMENT_TYPE_CREDITNOTE = "SRGC_GiftCard.creditnote";
  ...
  public static void updateGCPaymentTransaction(Order order, JSONObject jsonorder,
    ...
    for (int i = 0; i < payments.length(); i++) {
      ...
      if (!isPrePayment) {
        ...
        boolean returnOrder = Boolean.parseBoolean(payment.getString("isReturnOrder"));
        ...
        if (PAYMENT_TYPE_GIFTCARD.equals(kind[0])) {
          if(returnOrder) {
            gc.populateGiftCardTransactionReturn(giftCardTransaction, order, null, payment.getString("id"));
          }else {
