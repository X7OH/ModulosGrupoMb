...
(function () {
  ...
  OB.OBGCNE.Utils.PrintCreditNoteTemplate = '../ec.com.sidesoft.retail.giftcard/res/creditnote.xml';
  ...
  var findGiftCardModel = function (id, callback) {
      ...
      OB.Dal.find(OB.Model.GiftCardRetail, criteria, function successCallbackGiftCard(dataGiftCard) {
  ...
  var cancelGiftCard = function (keyboard, receipt, card, success, fail) {
      ...
      OB.UI.GiftCardUtilsRetail.service('ec.com.sidesoft.retail.giftcard.CancelGiftCard', {
        giftcard: card,
        _tryCentralFromStore: true,
        _executeInOneServer: true
      }, function (result) {
        OB.UI.GiftCardUtilsRetail.findProductModel(result.product.id, function (transactionproduct) {
  ...
  //*****************************************************************************************************//
  // 			FUNCION QUE SE EJECUTA AL APLICAR LA GIFTCARD Y AGREGA EL METODO DE PAGO AL RECIBO					   //
  //*****************************************************************************************************//      
  var consumeGiftCardAmount = function (keyboard, receipt, card, amount, includingTaxes, success, fail) {
      ...
      var modeloago = receipt.get('payments').models;

      var payment = _.find(receipt.get('payments').models, function (p) {
        return p.get('kind') === 'SRGC_GiftCard.Tarjetas:' + card.searchKey;
      });

      var giftCardPaymentMethod = _.find(OB.MobileApp.model.get('payments'), function (payment) {
        return payment.payment.searchKey === 'SRGC_GiftCard.Tarjetas';
      });

      OB.UI.GiftCardUtilsRetail.service('ec.com.sidesoft.retail.giftcard.ConsumeGiftCardAmount', {
        ...
        hasPaymentMethod: giftCardPaymentMethod !== undefined && giftCardPaymentMethod !== null
          ...
          var modelToApply = new OB.Model.PaymentLine({
            ...
            paymentData: {
              ...
              /*voidTransaction: function (callback, receipt, removedPayment) {
                ...
                //callback(false,null); // The payment has been voided successfully and the payment will be removed.
              }, */
              voidConfirmation: true
            },
            //name: OB.I18N.getLabel('SRGC_LblGiftCardsCertificat') + ' ' + card.searchKey,
            name: p.payment._identifier,
          ...
          keyboard.receipt.addPayment(modelToApply);
         ...
         OB.UI.GiftCardUtilsRetail.findProductModel(result.product.id, function (transactionproduct) {
            ...
            keyboard.doAddProduct({
              ...
              callback: function (success) {
                if (success === false) {
                  OB.UI.GiftCardUtilsRetail.cancelGiftCardTransaction(keyboard, result.transaction.id);
                }
              }
            });
          }); 
          ...
          console.log('******************************************');
          console.log('Se agrega pago',modelToApply);
          console.log('******************************************');
        } */
        if (success) {
          success();
  ...
  //*****************************************************************************************************//
  // 			FIN FUNCION QUE SE EJECUTA AL APLICAR LA GIFTCARD Y AGREGA EL METODO DE PAGO AL RECIBO				 //
  //*****************************************************************************************************//      

  //*****************************************************************************************************//
  //       FUNCION QUE HACE LA VALIDACION DE LA FECHA DE EXPIRACION DE LAS GIFTCARD					     //
  //*****************************************************************************************************//
  var checkIfExpiredGiftCardAndConsume = function (keyboard, receipt, card, amount, includingTaxes, success, fail) {
      ...
      OB.UI.GiftCardUtilsRetail.service('ec.com.sidesoft.retail.giftcard.FindGiftCard', {
        ...
        if (result && result.data && result.data.obgcneExpirationdate && OB.OBGCNE.Utils.isInThePast(result.data.obgcneExpirationdate)) {
           var errorMessage = {
            message: OB.I18N.getLabel('SRGC_GiftCardExpired', [card.searchKey]),
            keyboard: keyboard
          };
          fail(errorMessage);
        } else {
  ...
  //*****************************************************************************************************//
  //       	FIN FUNCION QUE HACE LA VALIDACION DE LA FECHA DE EXPIRACION DE LAS GIFTCARD				 //      
  //*****************************************************************************************************//      
      
  //*****************************************************************************************************//
  //       FUNCION QUE ME DESPLIEGA EL POPUP PARA REALIZAR LA BUSQUEDA DE LAS GIFTCARD					 //
  //*****************************************************************************************************//
  var consumeGiftCard = function (keyboard, amount, giftCardType) {
	  
    var header = giftCardType === 'BasedOnProductGiftCard' ? 'SRGC_HeaderGiftCard' : 'SRGC_HeaderGiftCard';
    
    keyboard.doShowPopup({
  	  popup: 'SRGC_UI_ModalGiftCard',
  	args: {
        keyboard: keyboard,
        ...
        action: function (dialog, consumeOK, consumeFail) {
          ...
          OB.UI.GiftCardUtilsRetail.checkIfExpiredGiftCardAndConsume(keyboard, keyboard.receipt, dialog.args.giftcard, amount, keyboard.receipt.get('priceIncludesTax'), successCallback, errorCallback);
        }
      }
    });
  };
  //*****************************************************************************************************//
  //       	FIN FUNCION QUE ME DESPLIEGA EL POPUP PARA REALIZAR LA BUSQUEDA DE LAS GIFTCARD				 //      
  //*****************************************************************************************************//      
  ...
  var consumeGiftCardLines = function (keyboard, receipt, card, success, fail, options) {
      ...
      OB.UI.GiftCardUtilsRetail.service('ec.com.sidesoft.retail.giftcard.ConsumeGiftCardLines', {
        giftcard: card,
        _executeInOneServer: true,
        _tryCentralFromStore: true,
        lines: lines,
        ...
        if (data && (data.exception || data.length === 0)) {
          ...
          if (options && options.cardType) {
            if (options.cardType === 'V') {
              params.header = OB.I18N.getLabel('SRGC_LblDialogGiftVoucher');
            }
          }
          keyboard.doShowPopup({
            popup: 'SRGC_UI_Message',
          ...
          _.each(data, function (item) {
            OB.UI.GiftCardUtilsRetail.findProductModel(item.product.id, function (transactionproduct) {
              // Add properties to product.
        ...
        if (options && options.cardType) {
          if (options.cardType === 'V') {
            errorMessage.header = OB.I18N.getLabel('SRGC_LblDialogGiftVoucher');
            errorMessage.message = OB.I18N.getLabel('SRGC_ErrorVoucherNotFound', [msgsplit.slice(1)]);
          }
        }
  ...
  var checkIfExpiredVoucherAndConsume = function (keyboard, receipt, card, success, fail, options) {
      OB.UI.GiftCardUtilsRetail.service('ec.com.sidesoft.retail.giftcard.FindGiftCard', {
        ...
        if (result && result.data && result.data.obgcneExpirationdate && OB.OBGCNE.Utils.isInThePast(result.data.obgcneExpirationdate)) {
          var errorMessage = {
            message: OB.I18N.getLabel('SRGC_GiftCardExpired', [card]),
            keyboard: keyboard
          };
          fail(errorMessage);
        } else {
  ...
  var cancelGiftCardTransaction = function (context, transaction) {
      ...
      // cancel transaction
      OB.UI.GiftCardUtilsRetail.service('ec.com.sidesoft.retail.giftcard.CancelGiftCardTransaction', {
        _executeInOneServer: true,
        _tryCentralFromStore: true,
